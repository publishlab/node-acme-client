 ...
 * @param {boolean} [saveNonce] Save nonce in queue if returned, default: `true`
 * @param {function} callback `{string}` err, `{object}` resp, `{object}` body
 */

HttpClient.prototype.request = function(uri, method, opts, saveNonce, callback) {
    if (typeof saveNonce === 'undefined') {
        saveNonce = opts;
        opts = {};
    }
    ...
    request(opts, function(err, resp, body) {
        ...
        if (saveNonce && resp.headers['replay-nonce']) {
            self.nonces.push(resp.headers['replay-nonce']);
        }
...
HttpClient.prototype.getNonce = function(callback) {
    debug('Nonce count: %d', this.nonces.length);

    if (this.nonces.length) {
        return callback(null, this.nonces.shift());
    }

    /* Request a fresh nonce */
    this.request(this.directoryUri, 'HEAD', {}, false, function(err, resp) {
        if (err) return callback(err);
